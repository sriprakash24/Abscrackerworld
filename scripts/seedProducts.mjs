...
async function seedProducts() {
  ...
  console.log(`Seeding ${items.length} products into Firestore project "${serviceAccount.project_id}"...`);
  ...
  for (let i = 0; i < items.length; i += chunkSize) {
    ...
    for (const p of chunk) {
      ...
      batch.set(ref, {
        ...
        salePrice: p.sale,
        discountPercentage: p.mrp > p.sale ? Math.round(((p.mrp - p.sale) / p.mrp) * 100) : 0,
        image: p.img,
        stock,
        stockQty,
        featured: false,
