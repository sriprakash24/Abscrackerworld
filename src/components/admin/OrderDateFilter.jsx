...
/**
 * Same calendar-with-counts multi-select UX as PackingInvoiceDateFilter on
 * the Packing/Delivery screens, generalised for the main Order Management
 * page — every order has a date here (not just confirmed ones), so this
 * filters on the same "best known date" (see getConfirmedDate) rather than
 * being invoice-specific.
 *
...
export default function OrderDateFilter({ options = [], selected = [], onChange }) {
  ...
  // Default the calendar to whichever month has the most recent order date,
  // so it opens already showing live data instead of an empty current month.
    ...
    <div ref={rootRef} className="relative shrink-0">
      ...
      {selected.length > 0 && (
        <button
          ...
          aria-label="Clear order date filter"
        >
